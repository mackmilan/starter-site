...
const defaultNavigationData: NavigationData = {
  ...
  footerDescription:
    "Uma base modular para apresentar sua organização, seus projetos e seus resultados.",
...
class NavigationService {
  ...
  async fetchNavigationData(): Promise<NavigationData> {
    try {
      ...
      const result: NavigationData = JSON.parse(
        JSON.stringify(defaultNavigationData),
      );

      if (
        menuRes.data &&
        Array.isArray(menuRes.data) &&
        menuRes.data.length > 0
      ) {
      ...
      console.warn(
        "Falha ao obter menu via API. Utilizando navegação padrão.",
        err,
      );
      return JSON.parse(JSON.stringify(defaultNavigationData));
    }
